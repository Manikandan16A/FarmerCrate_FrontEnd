...
class _OrderDetailPageState extends State<OrderDetailPage> {
  ...
  Future<void> _updateOrderStatus(String status) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text(errorData['message'] ?? 'Failed to update status'), backgroundColor: Colors.red),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  void _showStatusUpdateDialog() {
    ...
    if (options.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('No status updates available'), backgroundColor: Colors.orange),
      );
      return;
    }
